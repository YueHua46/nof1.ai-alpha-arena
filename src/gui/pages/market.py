...
def create_market(bot_service: BotService, state_manager: StateManager):
    ...
    try:
        ...
        # ===== ASSET SELECTOR =====
        with ui.row().classes('w-full items-center gap-4 mb-6 flex-wrap'):
            ...
            if isinstance(available_assets, str):
                available_assets = available_assets.split()
        ...
        # ===== PRICE CARDS =====
        with ui.row().classes('w-full gap-4 mb-6 flex-wrap justify-between'):
            # Current Price Card
            with ui.card().classes('metric-card flex-1 min-w-[220px]'):
        ...
        # ===== PRICE CHART =====
        with ui.card().classes('w-full p-4 mb-6 flex flex-col gap-4'):
        ...
        with ui.row().classes('w-full gap-4 mb-6 flex-wrap'):
            ...
            with ui.card().classes('flex-1 p-4 min-w-[280px]'):
        ...
        async def update_market_data():
            ...
            try:
                state = state_manager.get_state()
                if not asset_select or not asset_select.value:
                    return
        ...
        # Initial data fetch if bot is not running
        async def initial_market_load():
            """Fetch initial market data if bot is stopped"""
            if not bot_service.is_running():
                try:
                    # Show loading notification
                    # ui.notify('Fetching market data...', type='info')
                    await bot_service.refresh_market_data()
                    await update_market_data()
                except Exception as e:
                    ui.notify(f'Failed to fetch market data: {str(e)}', type='warning')
        
        # Schedule initial load
        asyncio.create_task(initial_market_load())
        
    except Exception as e:
        ui.label(f"Error loading market page: {str(e)}").classes('text-red-500 text-xl')
